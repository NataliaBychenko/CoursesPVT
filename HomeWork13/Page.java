 ...
 * - float id
 * - String title
 * Методы в классе Page:
 * Для каждой из коллекций в классе (4 шт) реализовтаь методы-обертки,
 * который будет принимать объект нужно типа и:
 * 1. Добавлять в коллекцию
 * 2. Удалять из коллекции
 * 3. Заменять элемент в коллекции
 * 4. Выводить все элементы коллекции
 */
...
public class Page {
    ...
    public boolean replace(Label replaceableLabel, Label newLabel) {
    ...
    public boolean add(Integer index, Dropdown newDropdown) {
        ...
        return dropdownHashMap.get(index).equals(newDropdown);
    ...
    public boolean replace(Dropdown replaceableDropdown, Dropdown newDropdown) {
        int key = 0;
        Iterator iterator = dropdownHashMap.entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry entry = (Map.Entry) iterator.next();
        ...
        return dropdownHashMap.containsValue(newDropdown);
    }

    public boolean replace(int key, Dropdown newDropdown) {

        dropdownHashMap.put(key, newDropdown);
        return dropdownHashMap.get(key).equals(newDropdown);
    }
    ...
    public void printAllDropdowns() {
        ...
        // System.out.println(dropdownHashMap);
    }
